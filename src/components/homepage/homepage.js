...
export default function Homepage() {
  ...
  function getTimeDiff(asleepTime, wakeupTime, entry) {
    ...
    let result = `${
      duration._data.hours < 10
        ? "0" + duration._data.hours
  ...
  return (
    <div
      id="background"
      className={`bg-gray-100 dark:bg-gray-900 splash-screen dark:splash-screen-dark transition duration-500 ${
        !displayModal && sleepArray.length < 5 ? "h-screen" : "h-full"
      } md:h-screen`}
    >
      <Navbar />
      <h1 className="text-3xl text-center font-bold text-black dark:text-white mt-10 mb-5 transition duration-500">
        {sleepArray.length !== 0
          ? "Your Sleep Entries"
          : "You haven't added any entries yet, you can add one by clicking the button below"}
      </h1>
      <div
        className={`${
          sleepArray.length < 1
            ? ""
            : "grid grid-cols-none md:grid-cols-2 mt-12"
        }`}
      >
        <div>
          <div className="flex justify-center">
            ...
            >
              <svg
                ...
                className="h-8 w-8 mr-2 my-auto"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div className="p-10 lg:p-24 bg-gray-200 dark:bg-gray-700 transition duration-500 rounded w-min mx-auto">
              <div className="mx-auto text-center align-middle w-64">
                <div>
                  <label
                    htmlFor="date"
                    className="font-bold text-black dark:text-white transition duration-500"
                  >
                    {" "}
                    Select the date
                    <TextField
                  ...
                  >
                    ...
                    <TextField
                      ...
                      className="w-60"
                      onChange={(event) => {
                        setAsleepTime(event.target.value);
                      }}
                      required
                    />
                  </label>
                  ...
                  <div className="flex">
                    <button
                      onClick={() => resetEntry()}
                      className="font-bold bg-gray-300 dark:bg-gray-500 m-auto px-5 hover:bg-gray-400 dark:hover:bg-gray-600 py-2 rounded transition-all duration-300"
                    >
                      Reset
                    </button>
                    <button
                      onClick={() => addSleepEntry()}
                      className="font-bold bg-yellow-400 m-auto px-4 hover:bg-yellow-500 py-2 rounded transition-all duration-300"
                    >
                      Submit
                    </button>
                  </div>
        ...
        <div className="totalArray text-xs md:text-lg text-black dark:text-white bg-gray-200 dark:bg-yellow-600 transition duration-500 rounded md:rounded-full justify-center">
          {" -- "}
          Only Show: {sleepArray.length} items{" -- "}
        </div>
        <div className="mx-10 mt-8 text-center">
          {sleepArray.map((entry) => (
            <div
              className="flex text-xs md:text-lg text-black dark:text-white justify-center bg-gray-200 dark:bg-yellow-600 w-full py-2 my-3 mx-auto opacity-90 transition duration-500 rounded md:rounded-full overflow-auto"
              key={entry.id}
            >
              <h1 className="mx-2 lg:mx-5 my-auto">
                <span className="font-bold">{entry.sleepDate}</span>
              </h1>
              <h1 className="mx-2 lg:mx-5">
                Asleep time:{" "}
                <span className="font-bold">{entry.asleepTime}</span>
              </h1>
              <h1 className="mx-2 lg:mx-5">
                Wakeup time:{" "}
                <span className="font-bold">{entry.wakeupTime}</span>
              </h1>
              <h1 className="mx-2 lg:mx-5">
                <span className="font-bold">
                  {getTimeDiff(entry.asleepTime, entry.wakeupTime, entry)} Hours
                  Slept
                </span>
              </h1>
              <span
                className="cursor-pointer my-auto hover:text-red-600 mr-2"
                onClick={() => {
