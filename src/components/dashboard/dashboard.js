...
function Dashboard() {
  ...
  const [goodSleep, setGoodSleep] = useState(0);
  ...
  function getSleepStats() {
    ...
    for (let i = 0; i < sleepArray.length; i++) {
      ...
      var minutes = hoursMinutesTotalSlept[1]
        ? parseInt(hoursMinutesTotalSlept[1], 10)
        : 0;
    ...
    sleepArray.map((entry) => {
      if (parseInt(entry.totalSlept) < 6) {
        ...
        return goodSleep.push(parseInt(entry.totalSlept));
      }
    });

    setLess6Hours(less6Hours.length);
    setMore8Hours(more8Hours.length);
    setGoodSleep(goodSleep.length);
  }

  return sleepArray.length > 0 ? (
    <div className="h-full md:h-screen splash-screen dark:splash-screen-dark transition duration-500">
      <Navbar />
      <div className="text-center">
        <h1 className="font-bold text-3xl lg:text-5xl my-8 text-black dark:text-white transition-all duration-500">
          Your Dashboard
        </h1>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 mx-10 my-12 md:my-0 lg:my-24 relative">
          ...
          >
            ...
            <LineSeries
              data={sleepArray.map((entry) => {
                return {
                  x: new Date(entry.sleepDate),
                  y: parseInt(entry.totalSlept),
                };
              })}
              style={{ strokeWidth: 3 }}
            />
        ...
        <div className="text-center justify-center m-5 w-full mx-auto">
          <div
            className="bg-gray-400 dark:bg-gray-700 opacity-80 rounded-lg py-5 text-black dark:text-white transition-all duration-500"
            style={{ height: 500 }}
          >
            <h1 className="text-3xl font-bold mb-3 text-black dark:text-yellow-500 transition-all duration-500">
              Stats
            ...
            <h1 className="text-lg md:text-xl lg:text-xl font-bold mt-10 mx-5">
              {averageSleepTime < 6
                ...
                : averageSleepTime > 8
                ? "You sleep too much. Healthy adults need between 6 and 8 hours of sleep"
                : "You are having the recommended amount of sleep. \n Keep it up!"}
            </h1>
          </div>
        </div>
      </div>
    ...
    <div className="h-full md:h-screen splash-screen dark:splash-screen-dark transition duration-500">
      ...
      <div className="text-center">
        ...
        <h1 className="font-bold text-3xl lg:text-5xl mt-48 mx-5 text-black dark:text-white transition-all duration-500">
          You don't have any entries yet,{" "}
          <Link to="/entries" className="text-yellow-500">
            click here
          </Link>{" "}
          to add some
        </h1>
